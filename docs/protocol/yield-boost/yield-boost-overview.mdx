...
Linea Yield Boost is a protocol-level mechanism that automatically stakes some of the ETH held in the Linea bridge, or LineaRollup, and distributes the staking rewards back into the Linea ecosystem. When a user bridges ETH from Ethereum (L1) to Linea (L2) via the canonical bridge, a portion of these assets is programmatically deposited into a dedicated Lido v3 stVault on Ethereum Mainnet. The resulting staking rewards are distributed back into the Linea ecosystem to incentivize liquidity providers LPs and support DeFi protocols.

The user experiences no change in their ETH balance or typical user interface on Linea. The staking process occurs in the background on L1 within the canonical bridge contracts. To ensure users can always withdraw and transact with their funds, the system maintains a Liquidity Buffer of unstaked ETH specifically to service withdrawals. This amount targets around 40% of the total value staked, and the system automatically rebalances funds between the stVault and the buffer to maintain sufficient liquidity.

This page maps the high-level architecture of Yield Boost, including where ETH flows and which roles control each movement. For a more general overview of Yield Boost, see the [Yield Boost overview](/protocol/yield-boost/) page.

## High-level architecture

LineaRollup deposits on Ethereum L1 are staked via Lido V3 stVaults to generate beacon chain rewards, which are reported to L2 for distribution. The following is a breakdown of the major components and flows of the Yield Boost system.

### L1 ETH flow

For ETH to flow from the user to the beacon chain, and back to the user, the following components are involved:

- **LineaRollup** - Main Linea bridge contract on L1; holds user-deposited ETH and receives yield back after staking.
- **YieldManager** - L1 contract that moves ETH between the LineaRollup and the StakingVault, and calculates net yield after fees and liabilities.
...
- **Node operator** - Runs beacon chain validators on behalf of Linea and decides when to deposit ETH from the StakingVault into the beacon chain.
- **Automation service** - Offchain bot that triggers routine operations: moving ETH to/from staking, topping up reserves, and reporting yield.
...
The yield earned from the beacon chain is reported to L2 for distribution. The following components are involved:
...
1. **Staking** (routine) - the Automation Service rebalances surplus ETH from the LineaRollup into the StakingVault for validator staking.
...
Surplus ETH in the LineaRollup (above minimum reserve) is routed to the StakingVault for beacon chain staking.
...
Replenishing the withdrawal reserve is a two-phase process: first trigger beacon chain withdrawal, then route funds to the LineaRollup once they arrive in the vault.
...
When the LineaRollup balance drops below the minimum reserve, anyone can trigger unstaking and reserve replenishment.
...
If the LineaRollup lacks ETH for a user withdrawal, stETH is minted against the StakingVault collateral and sent directly to the user as a last resort.
...
Ossification permanently locks the StakingVault implementation, opting out of future upgrades. The Security Council initiates it; the Automation Service then progressively withdraws all staked funds back to the LineaRollup.
