...
- **LineaRollup** - L1 LineaRollup contract that holds user deposits and receives ETH back from staking flows.
...
- **Lido** - Broader Lido protocol (not a single contract).
...
- **Fee Recipient** - Address that receives fees on behalf of the node operator.
...
- **Security Council** - Linea Security Council Multisig.
...
flowchart TD
    ...
    YM -->|fund| DB
    DB -->|fund| SV
    NO -.->|decides when to trigger deposits| SV
    SV -->|stake| BC
    BC -->|withdrawal| SV
    SV -->|withdraw| DB
    DB -->|withdraw ETH| LR
