...
Linea Yield Boost is a protocol-level mechanism that automatically stakes some of the ETH held in the Linea bridge, or LineaRollup, and makes those staking rewards available on L2 for distribution. This page maps the high-level architecture, including where ETH flows and which roles control each movement.
