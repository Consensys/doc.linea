...
The example configuration shown in the image is applied by [Linea's Public Mainnet](/network/index.mdx). Notice that while the protocol itself is agnostic about where finalization occurs. The example configuration supports transaction finalizion as proofs submitted on, and data (temporarily) made available to, Ethereum Mainnet through a zero-knowledge rollup. 
