...
6. Data submission: Lineth contracts define three data-submission models:
   - Compressed data submitted as EIP-4844 blobs. Supported by the Coordinator. The finalization contract must implement `IAcceptEip4844Blobs`.
   - Compressed data submitted as calldata. The current Coordinator does not provide a calldata submitter. The finalization contract must implement `IAcceptCalldataBlobs`.
   - Shnarf-only submission for validium deployments, without publishing the compressed batch data to the finalization layer. Supported by the Coordinator. The finalization contract must implement `IAcceptShnarfData`.
