...
Full nodes maintain current network state and participate in consensus:
...
Data availability (DA) and finalization are separate concerns:

- DA: Where transaction data is stored (onchain vs. offchain)
- Finalization: zk-SNARK proofs attest
to correct execution so finalization refers to where proofs and state commitments are verified (L1 vs. L2)

DA guarantees depend on the chosen [deployment model](../../stack/how-it-works/deployment-models/index.mdx):

- Public deployments: transaction data and associated payloads are posted to the finalization layer, for example via EIP-4844 blobs on Ethereum.

- Private Validium deployments: transaction data is retained offchain within a private data availability
  node set; only proofs and finalized state
  roots are submitted to the finalization layer.

- Hybrid deployments: DA varies by transaction type.

:::tip

For Public Mainnet's protocol-level context on batching, finality, and data publication, see
[transaction lifecycle](../transaction-lifecycle.mdx) and
[network data](../../network/build/network-data.mdx).

:::

## Signing services

Operators that run this architetural stack typically leverage a signing service that externalizes private key management from the nodes themselves. This allows the clients to obtain signatures on blocks, attestations, and transactions without directly holding the private keys. This allows the components participating in block production and finality to integrate with signing services to sign consensus messages and transactions. 

:::important

Always ensure that private keys are stored and protected using a Key Management Service (KMS) or a secure alternative. It's best practice to ensure that signing operations are centralized, access‑controlled, and auditable.
