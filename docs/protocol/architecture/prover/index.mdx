...
The module responsible for generating the final set of data for a <GlossaryTerm term="Zero-knowledge proof">zero-knowledge proof</GlossaryTerm>, and producing that proof.
...
- Outer proof: is generated with the Consensys-maintained
  [gnark library](../../../protocol/reference/repos.mdx#gnark), further compressing the result into
  a <GlossaryTerm term="zk-SNARK" />.
...
This provides cryptographic guarantees for state correctness without requiring third parties to
re-execute transactions. The proof verifies correctness for transaction batches before being accepted by the
finalization layer. This process enables the protocol to provide secure, efficient, and cost-effective
transaction scaling while reducing trust assumptions around execution correctness.
