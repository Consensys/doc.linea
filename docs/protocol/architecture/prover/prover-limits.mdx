...
<GlossaryTerm term="Lineth" /> uses module limits to keep proof complexity manageable and ensure that the prover can
generate proofs efficiently without overwhelming the infrastructure. 

Lineth's arithmetization specification is divided into modules, and each module represents a collection
of <GlossaryTerm term="Ethereum Virtual Machine (EVM)">EVM</GlossaryTerm> or Lineth <GlossaryTerm term="zkEVM" /> operations (opcodes). Modules can:
...
[Lineth source code](https://github.com/LFDT-Lineth/lineth-monorepo/blob/main/docker/config/common/traces-limits-v5.toml). 
