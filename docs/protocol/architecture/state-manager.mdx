...
The state manager consists of a Besu node with [Shomei](../../protocol/reference/repos.mdx#shomei)
plugin and a Shomei
...
The state manager maintains the network's state representation optimized for
proof generation:
...
- Serves [`linea_getProof`](../../api/reference/linea-getproof) RPC endpoints

The state manager is the part of the execution client responsible for updating the state of the 
network globally, and the state of every account individually.  It tracks the current state of user accounts, smart contracts, and balances on the network. The state manager also audits the
...
The main task of the state manager is to receive blocks that have been executed by the 
[sequencer](./sequencer/index.mdx) and use the trace data from their execution to update the state of the network. 

## How does it do it?

Lineth uses two data structure types to manage state:
...
2. A variant of a regular [Merkle tree](./state-manager.mdx#merkle-trees) called a sparse Merkle tree (SMT), which is used to more
...
in the form of Merkle proofs for submission to the <GlossaryTerm term="Finalization layer">finalization layer</GlossaryTerm>. 

Below, we'll explain the element of Lineth state management in greater detail, focussing on the
SMT configuration that sets Lineth apart.
...
As a rollup stack, Lineth is in the business of making transacting faster and more efficient.
Lineth implements a sparse Merkle tree to track account state and generate and store proofs, and 
...
Lineth's state management uses sparse Merkle trees to minimize computation and contribute to the 
...
Lineth adapts this structure. The Merkle-Patricia Trie is still used for world state, but the
...
interpretation. The data must be formatted this way to enable the prover to correctly 
