...
network globally, and the state of every account individually.  It tracks the current state of user accounts, smart contracts, and balances on the Linea Network. The state manager also audits the
