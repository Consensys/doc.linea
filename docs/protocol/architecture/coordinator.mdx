...
The coordinator achieves this through a highly modularized internal architecture. These various systems and requirements are broken out into their own environments, each one receiving input from the coordinator, and returning the corresponding output back to it.
