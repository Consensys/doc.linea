...
One of the most important things that the message service transfers is information about the current state of the Ethereum network, from Ethereum to Linea, and in return, an updated [Merkle tree](../state-manager.mdx#merkle-trees) and a <GlossaryTerm term="Zero-knowledge proof">ZK proof</GlossaryTerm> from Linea to Ethereum, every time Linea reports back about activity on the network. In other words, the canonical message service transmits the rollup data.
...
The canonical message service consists of three main elements: two smart contracts, and the postman service in between. As you may have guessed already, the smart contracts are [on Linea and Ethereum](../../../network/build/send-receive-messages.mdx#contracts), and are almost exactly the same. They allow for ETH to be minted on the target network, for example, though they are not limited to that.
