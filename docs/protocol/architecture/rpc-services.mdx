...
Standard Ethereum JSON-RPC namespaces are available on any execution client. The `linea_` namespace is
implemented by Linea Besu plugins, so a node serves those methods only if it runs Linea Besu with the
relevant plugins enabled. A node running Geth, Erigon, Nethermind, or upstream Besu serves standard
Ethereum methods but not the `linea_` namespace.
