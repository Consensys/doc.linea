...
Conflation is the process of combining two or more blocks’ worth of transactions into one data set. It can then be used to produce a ‘before and after’ map of the network state (a [Merkle tree](../state-manager.mdx#merkle-trees), in technical terms) as well as a zero-knowledge proof, which is published to Ethereum.
