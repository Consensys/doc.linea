...
If a validator makes conflicting attestations or block proposals, they may have their staked ETH confiscated, and the staking rewards lost. This is known as slashing, and is a general risk associated with staking. 

The risk of slashing is rare: only 0.04% of all Ethereum validators have ever been slashed, and the slashed amounts are on average less than .001% of that validator's staked ETH.

Even with this low risk, the Yield Boost system has the following mitigations in place:
...
- 20% of node operator fees will be retained for a year to create an insurance fund to cover slashing events. Any user in the Yield Boost system impacted by a slashing event will be eligible for this fund.
...
- As a last resort, users can [withdraw their funds as stETH](/protocol/yield-boost-architecture#5-lst-withdrawal----last-resort) rather than ETH.
