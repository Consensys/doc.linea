...
[Linea Yield Boost](../network/overview/yield-boost/) is a protocol-level mechanism implemented on the [Public network](../network/quickstart) that automatically stakes a proportion of the ETH held in the [canonical token bridge](../network/build/contracts.mdx#token-bridge-contracts) and distributes the staking rewards back into the Linea ecosystem. When a user bridges ETH from Ethereum (L1) to Linea (L2) via the canonical token bridge, these assets may be programmatically deposited into a dedicated [Lido v3 stVault](https://docs.lido.fi/lido-v3-whitepaper) on Ethereum Mainnet. The resulting staking rewards are distributed back into the Linea ecosystem to incentivize liquidity providers (LPs) and support DeFi protocols.
...
Canonical token bridge deposits on the L1 are staked via a Lido V3 stVault to generate beacon chain rewards, which are [distributed on Linea (L2)](../network/overview/yield-boost.mdx#why-yield-boost-exists). The following is a breakdown of the major components and flows of the Yield Boost system.
...
- **Node operator:** The Linea-assigned entity bound to the agreed deposit trigger to stake ETH from the stVault, and validate the beacon chain to earn rewards.
...
When a user deposits ETH from Ethereum Mainnet (L1) into the canonical token bridge contract to use it on Linea (L2), that ETH remains on the L1 and may be staked on Ethereum’s beacon chain to earn rewards. The following components manage how that ETH moves from the bridge contract into staking, how rewards are handled, and how the ETH is returned to the user on the L1 when they withdraw it from Linea.

- [**Canonical token bridge:**](../network/build/contracts.mdx#token-bridge-contracts) L1 bridge contract that holds user-deposited ETH collateral and serves as the accounting anchor for Yield Boost.
- **`YieldManager`:** L1 contract that moves ETH between the canonical token bridge and the `StakingVault`, and calculates net yield after fees and liabilities.
- **`StakingVault`:** Lido V3 stVault that holds ETH awaiting beacon chain staking; all validator withdrawals return here before moving back to the canonical token bridge.
...
flowchart TD
    ...
    Auto -.->|Automatically rebalances ETH between canonical token bridge and StakingVault, and requests unstaking| YM
...
- **`LineaRollup`:** L1 rollup and message service contract that emits synthetic `MessageSent` events to relay net yield to the L2.
...
6. The `LineaRollup` emits a synthetic `MessageSent` event. The `L2MessageService` picks it up and unlocks the corresponding ETH on L2.
...
1. **Staking** (routine) -- the Automation Service rebalances surplus ETH from the canonical token bridge into the `StakingVault` for validator staking.
2. **Reserve replenishment** (routine) -- the Automation Service rebalances ETH back from validators into the canonical token bridge when the withdrawal reserve runs low.
...
5. **LST withdrawal** (last resort) -- if the canonical token bridge does not control enough ETH to fulfill a user's withdrawal, the user can claim stETH instead.
...
Surplus ETH in the canonical token bridge (above the minimum reserve) is routed to the `StakingVault` for beacon chain staking.

```mermaid
sequenceDiagram
    actor Auto as Automation Service
    participant BR as Canonical token bridge
    ...
    Auto->>BR: transferFundsForNativeYield()
    BR-->>YM: send ETH
...
Replenishing the withdrawal reserve is a two-phase process: first the beacon chain withdrawal is triggered, then funds are routed back to the canonical token bridge once they arrive in the vault.
...
When the canonical token bridge balance drops below the minimum reserve, anyone can trigger the `YieldManager` to initiate unstaking and reserve replenishment.
...
sequenceDiagram
    ...
    participant SV as StakingVault
    participant BC as Beacon chain
    Note over BR: Reserve in deficit

    rect rgba(230, 240, 255, 0.6)
        Note over BR: Phase 1 - Unstake
    ...
    rect rgba(240, 255, 230, 0.6)
        Note over BR: Phase 2 - Replenish withdrawal reserve
...
If the canonical token bridge lacks sufficient ETH to fulfill a withdrawal, the withdrawal recipient can explicitly request an LST withdrawal. In this case, stETH is minted against the `StakingVault` collateral and sent to the same address designated to receive the withdrawal. This provides a last-resort mechanism.
...
sequenceDiagram
    ...
    Note over BR: Insufficient ETH for withdrawal
    U->>BR: claimMessageWithProofAndWithdrawLST()
    BR->>YM: withdrawLST()
    YM->>DB: mintStETH()
    DB-->>U: send stETH
...
Ossification permanently locks the `StakingVault` implementation, opting out of future Lido upgrades. Ossification is initiated by the Security Council; the Automation Service then progressively withdraws all staked funds back to the canonical token bridge.
...
|--------------|--------|-------------|---------|---------------|
| Stake excess reserve | Canonical token bridge | `StakingVault` | Automation Service | `YIELD_PROVIDER_STAKING_ROLE` |
...
| Operator replenish reserve | `StakingVault` | Canonical token bridge | Reserve below target | `YIELD_PROVIDER_UNSTAKER_ROLE` |
| Permissionless unstake | Validators | `StakingVault` | Reserve below minimum | Permissionless |
| Permissionless replenish reserve | `StakingVault` | Canonical token bridge | Reserve below minimum | Permissionless |
...
| Ossification withdrawal | `StakingVault` | Canonical token bridge | Security Council initiates | `OSSIFICATION_INITIATOR_ROLE` + `OSSIFICATION_PROCESSOR_ROLE` |
| Donation | External | Canonical token bridge / `StakingVault` | Voluntary | Permissionless |
