...
`LineaRollup` deposits on the L1 are staked via a Lido V3 stVault, known as the `StakingVault, to generate beacon chain rewards, which are [distributed on Linea (L2)](../network/overview/yield-boost.mdx#why-yield-boost-exists). The following is a breakdown of the major components and flows of the Yield Boost system.
...
- **Node operator:** The Linea-assigned entity bound to the agreed deposit trigger to stake ETH from the `StakingVault`, and validate the beacon chain to earn rewards.
