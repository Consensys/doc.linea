...
The following example executes the `linea_estimateGas` API on the Linea Goerli testnet.
...
- `priorityFeePerGas` - Calculates the fee required to prioritize a transaction by considering factors such as the
    ...
    layer 2, the transaction's gas usage, the minimum minable gas price on Layer 2, and a minimum
