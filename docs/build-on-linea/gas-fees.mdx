...
- Transactions won't be sequenced if the `gasPrice` or `maxPriorityFeePerGas` falls below a certain threshold, which adjusts
    over time, transactions are added to the pending queue until the gas price on the network drops sufficiently for the
...
You can [use the `linea_estimateGas`](#calculate-transaction-fees) gas API to estimate your gas costs; the result of the
call provides an estimation that includes the layer 1 and layer 2 fees.

## Calculate transaction fees

Use the `linea_estimateGas` API to estimate the gas cost for sending a transaction. The API uses the same inputs as the
standard [`eth_estimateGas`](https://docs.infura.io/api/networks/linea/json-rpc-methods/eth_estimategas), but returns the
recommended gas limit, the base fee per gas, and the priority fee per gas, providing a more comprehensive estimate for
executing transactions on the Linea network.
...
- `priorityFeePerGas` - Uses the following formula:
    ...
    - `L2_compressed_tx_size_in_bytes`: This is the size of the transaction data after it has been compressed using the
        `gnarkCompression` function (the zk-SNARK library used by Linea). It's calculated by taking the size of the
