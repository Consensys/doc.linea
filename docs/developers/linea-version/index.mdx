...
Reducing block time to 2 seconds, with a target block size of 24M gwei.

The change will increase transaction throughput by approximately 20%, and therefore improve user
experience by making Linea feel faster and more responsive.
...
The [Linea bridge](https://bridge.linea.build/) has been upgraded: when an address is specified as
the recipient that is different from the address of the sender, the recipient address is now able
to send the claim message on the destination chain. 
...
their claiming outside of the Bridge UI, then we recommend looking at the updated [SDK](/developers/guides/linea-sdk).
...
Major changes are applied to the Canonical Token Bridge as described in our [documentation](/architecture/stack/bridges/canonical-token-bridge).
