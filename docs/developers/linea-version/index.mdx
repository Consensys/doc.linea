...
- Adds the ability to detect whether transactions will exceed the prover's computational limit. 
Transactions that previously would have been rejected by the sequencer but accepted by nodes will 
now fail when appropriate, rather than staying in limbo. 
- Reactivates `linea_estimateGas`.
- Linea Bridge: adds the ability to bridge to a different address, enabling the recipient to claim.
- Implemented smart contract gas optimization, reducing the total gas units required per blob
submission. 
