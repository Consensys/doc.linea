...
integrations) for receiving, transforming, storing, and accessing your on-chain
data.
...
1. Clone the
   [starter boilerplate](https://github.com/flair-sdk/starter-boilerplate)
...
3. Sync some historical data using the
   [`backfill` command](https://docs.flair.dev/reference/backfilling). Remember
   that the `enabled: true` flag in your `config` already enables your indexer
   to capture data in real-time.
