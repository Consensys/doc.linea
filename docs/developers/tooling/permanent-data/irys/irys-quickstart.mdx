...
Additionally, these can be [used with extra setup code](https://arweave-tools.irys.xyz/irys-sdk/irys-in-the-browser):
...
The [fee is based on the number of bytes uploaded](https://arweave-tools.irys.xyz/overview/cost-to-upload).
...
Irys supports adding any optional [metadata tags](/developers/tooling/permanent-data/irys/overview#tagging) to each upload. When uploading files with a filename extension, the related [`Content-Type`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Type) (MIME type) tag is automatically added.
...
When you upload data to Irys, you're given a [receipt](/developers/tooling/permanent-data/irys/overview#receipts) containing a transaction ID. Use this ID to download your data from the Irys gateway by creating a URL in the format:
