...
When you use Irys to upload data, you pay once and the data is guaranteed to be retrievable forever. The [fee is based on the number of bytes uploaded](https://docs.irys.xyz/build/d/sdk/payment/getPrice).
...
Irys supports adding any optional [metadata tags](./overview.mdx#tagging) to each upload. When uploading files with a filename extension, the related [`Content-Type`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Type) (MIME type) tag is automatically added.
