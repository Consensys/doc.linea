...
Linea. With a single upfront payment, you can upload data, with guaranteed
retrieval for a duration you set at the upload time — from a few days to
forever.
...
const getIrysUploader = async () => {
  const irysUploader = await Uploader(LineaEth).withWallet(
...
> The following code example is for Ethers v6. For other providers,
> [see Irys's docs](https://docs.irys.xyz/build/d/irys-in-the-browser).
...
const getIrysUploader = async () => {
  ...
  const irysUploader = await WebUploader(WebLineaEth).withAdapter(
...
retrievable forever. The
[fee is based on the number of bytes uploaded](https://docs.irys.xyz/build/d/sdk/payment/getPrice).
...
To use Irys's devnet where uploads are kept for ~60 and you pay with Linea
...
Use the `-t` option, followed by a series of name / value pairs to append
metadata tags to your upload.
...
Use the `price` command, followed by a number of bytes to get the cost to upload
that number of bytes. You must also provide a token (`-t`) and a node URL (`-h`)
