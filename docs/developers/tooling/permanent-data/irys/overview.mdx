...
verify what data was posted, when, and where it's located in Irys’s data
ledgers.
...
- Deploy NFT contracts on Linea and
  [have images and metadata on Irys](../../../guides/community/irys/irys-nfts.mdx)
...
You can use
[ETH on Linea](https://arweave-tools.irys.xyz/overview/supported-tokens) to pay
for permanent storage on Irys.

## Paying for uploads

Irys has a pay-once-store-forever payment model based on the number of bytes you
upload.
...
Use
[balance approvals](https://docs.irys.xyz/build/d/features/balance-approvals) to
pay for your users' uploads and build a Web2-style UX that saves users from
...
Transaction metadata can be queried using the
[Irys's GraphQL](../../../tooling/permanent-data/irys/irys-querying.mdx).
