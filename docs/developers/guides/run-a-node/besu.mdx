...
:::info important Install and run a Besu node if you want to follow the Linea network by maintaining a local copy of the blockchain. However, if you want to interact with the network and use Linea-specific methods and features, you should [install Linea Besu](./linea-besu.mdx) instead. :::
...
:::note[bootnodes] You can choose from a range of bootnodes for Linea Mainnet. The above configuration file uses all bootnodes by default.
...
In your Besu configuration file (`config-mainnet.toml` or `config-sepolia.toml`), configure the following options:
...
:::note[bootnodes] You can choose from a range of bootnodes for Linea Mainnet. The Besu `.zip` includes a config file named `config-snap-mainnet.toml` where bootnodes are specified. The file uses all bootnodes by default.
