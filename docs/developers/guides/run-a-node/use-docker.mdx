...
<Tabs groupId="networks" className="my-tabs">
  ...
  config file named `config-snap-mainnet.toml` where enodes are specified. The file uses US-based 
  enodes by default, but you may experience better performance if you choose enodes based in a 
  closer region (Europe or Asia-Pacific). 
...
<Tabs groupId="networks" className="my-tabs">
  ...
  file uses US-based enodes by default, but you may experience better performance if you choose 
  enodes based in a closer region (Europe or Asia-Pacific). 
  ...
  Download the Goerli testnet [`docker-compose.yml`](pathname:///files/geth/goerli/docker-compose.yml) and [`genesis.json`](pathname:///files/geth/goerli/genesis.json)
  files.
