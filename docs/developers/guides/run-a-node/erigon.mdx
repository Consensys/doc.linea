...
:::info important Install and run an Erigon if you want to follow the Linea network by maintaining a local copy of the blockchain. However, if you want to interact with the network and use Linea-specific methods and features, you should [install Linea Besu](./linea-besu.mdx) instead. :::
...
:::info Ensure you review [Erigon's software prerequisites](https://erigon.gitbook.io/erigon/basic-usage/getting-started#software-prerequisites) before installing the Erigon client.
...
Download the genesis file for the relevant network. In the example, we'll download the file to the directory specified by [`datadir` in Step 4](#step-4-bootstrap-your-node).
...
If you run out of space, or need to actively maintain how much space is being used, consider [pruning](https://erigon.gitbook.io/erigon/advanced-usage/options).
...
:::note[bootnodes] You can choose from a range of bootnodes for Linea Mainnet. The above command uses all bootnodes by default.
