...
Linea Besu is an implementation of the Besu client that extends its functionality using plugins adapted specifically for Linea, such as the [Linea Sequencer plugin](https://github.com/Consensys/linea-sequencer).
...
[Download the latest version](https://github.com/Consensys/linea-besu-package/releases) of the Linea Besu package from the releases page.

:::note Find the **Assets** subheading on the latest release and download the file named `linea-besu-package-<version>.tar.gz`. :::
...
Access the [`/docker` directory](https://github.com/Consensys/linea-besu-package/tree/main/docker) in the Linea Besu repository. There are several `.yaml` files here corresponding to Besu profiles. Each profile enables you to run a node with different Linea Besu plugin configurations depending on your use case.
