...
`linea_estimateGas` enables its users to reproduce on demand the transaction order logic that the 
sequencer applies to all transactions when building blocks, ensuring that the transaction price has 
enough of a buffer for inclusion. This logic involves selecting transactions for the priority fees
they pay, a system known as a priority gas auction, but also ensuring the priority fees offered by
each transaction are high enough to support the network's sustainability, and cover L1 data costs.
