...
4. Select one of the available tokens in the list. If the token is not in the list, use the search bar to find it by contract address, symbol, or its full name. Click on your preferred token to select it.
...
When an ERC-20 token is bridged from one chain to the other and the token has not been deployed on the destination chain, the Token Bridge Smart Contract will attempt to determine the “decimals” of the ERC-20 token. If the decimals cannot be determined by reading the origin ERC-20 token contract (e.g. a token contract doesn't adhere to the [ERC-20 standard](https://ethereum.org/en/developers/docs/standards/tokens/erc-20/)), it will default to 18 decimals.
