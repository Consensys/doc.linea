...
Linea's default cononical token bridge allows you to bridge ETH or ERC-20 tokens using a UI.

The default cononical token bridge is suited for protocols that need their tokens on L2 for
...
the [cononical token bridge](../../../architecture/stack/bridges/index.mdx).
:::

Select one of the following cards for instructions to bridge using the default cononical token bridge.
