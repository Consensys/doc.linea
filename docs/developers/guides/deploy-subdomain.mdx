...
addresses deployed for the `linea.eth` suddomain, contact [Linea Support](https://support.linea.build/)
...
1. Copy the `PublicResolver` contract address in the first line of the
    `./deployments/lineaMainnet/PublicResolver.json` file. For example:

    ```
    "address": "0x86c5AED9F27837074612288610fB98ccC1733126",
    ```

    Copy this address because you'll need it in the next step.
