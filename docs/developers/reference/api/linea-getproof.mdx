...
finalised block on the L1, users must retrieve the value from the `currentL2BlockNumber` variable in
the [LineaRollup L1 contract](https://etherscan.io/address/0xd19d4b5d358258f05d7b411e21a1460d11b0876f#readProxyContract).
:::
...
- `accountProof`: Provides proof data for the account:
  ...
  - `leafIndex`: The index of the leaf node in the Merkle Patricia Trie that corresponds to the account.
  - `proof`: Object containing the proof data for the account:
    - `proofRelatedNodes`: Array of nodes in the Merkle Patricia Trie used to construct the proof.
...
- `storageProof`: An array of storage-entries as requested. Each entry is an object with these properties:
  ...
  - `leftProof`: Contains the proof data for the left leaf node, which includes the array of nodes used
      to construct the proof and the value stored at the left leaf node in the Merkle Patricia Trie.
  ...
  - `rightProof`: Contains the proof data for the right leaf node, which includes the array of nodes used
      to construct the proof and the value stored at the right leaf node in the Merkle Patricia Trie.
