...
The difference lies in the [error responses](#errors). On Linea, `eth_sendRawTransaction` has some unique error responses.
...
The error `Gas price too low` is thrown when the transaction's gas is too low; specifically, the offered gas doesn't create enough of a buffer to ensure that Linea can cover the costs of finalizing the transaction on L1.

### Transaction line count above limit

This error will display in the format: `Transaction line count for module [A] is above the limit [B]`

Where `[A]` is the module for which the number of lines is above the maximum allowed value `[B]`, with `[B]` a parameter of the system.
