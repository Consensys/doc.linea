...
Queries a temporary database maintained by the transaction exclusion API service
to check if a transaction was rejected by the sequencer, connected P2P node, or
RPC nodes for exceeding data line limits that would prevent the prover from
generating a proof.

:::info

You can only check for transaction rejection within seven days of the
transaction attempt. Querying transactions older than this will return a `null`
response.

:::

If the transaction is rejected, the API call will succeed and provide the
reason; otherwise, it will return `null`.
...
- `txRejectionStage`: A string detailing the point at which the transaction was
  rejected. One of:
...
- `blockNumber`: The block that the transaction was rejected from, in
  hexadecimal format. Only returned for transactions rejected by the sequencer.
...
This example is purely representative; the database retains transaction data for
only seven days, so you'll be unable to reproduce this response with the above
example.

:::

If a transaction was not rejected for exceeding line limits, it will not be
...
A similar `null` result is also returned if the transaction is older than seven
days.
