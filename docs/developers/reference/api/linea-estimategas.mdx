...
The `linea_estimateGas` is only fully compatible with endpoints using the Besu client — if it is 
unavailable, try updating Besu. If you prefer to use Geth, `linea_estimateGas` is only available if 
you redirect your request through an Infura endpoint.
...
Here is an example of how to call `linea_estimateGas` using the ethers.js library:
