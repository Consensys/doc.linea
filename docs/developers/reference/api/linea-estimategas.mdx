...
`linea_estimateGas` is being reactivated, but under the following conditions:
- Available only on testnet initially, from May 29, 2024.
- Compatible only when interacting with the blockchain via an endpoint using the Besu client. 
`linea_estimateGas` will not work with endpoints using Geth.
...
Hexidecimal values representing the recommended gas limit, the base fee per gas, and the priority 
fee per gas.
