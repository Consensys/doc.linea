...
description: Deploy a private Validium network with offchain data availability
sidebar_position: 1
mainSource:
...
See [Deploy execution layer](./deploy-execution-layer.mdx) for detailed
steps.
...
- Set up key management: Remote signing backed by a hardware security module (HSM) or key management (for example, Web3Signer with KMS)
...
3. Test transaction submission and execution (see [Transaction lifecycle](../../../protocol/transaction-lifecycle.mdx) for
   expected flow).
