...
For access policy, see [Access control](./rbac.mdx).
...
- Cryptographic private execution or private state. Lineth does not publish a
  ...
  RBAC.
- Regulatory compliance. See [Compliance](./compliance.mdx) for what protocol
  evidence exists and what is not claimed.
- Access policy configuration. See [Access control](./rbac.mdx).
...
| RPC and API surfaces             | State and transactions the caller is authorized to view | Callers with [RBAC](./rbac.mdx) permissions for the relevant endpoint        |
...
deployment's [RBAC](./rbac.mdx) and governance rules control.
...
See
[a comparison of finalization layer options](../how-it-works/data-availability-finalization.mdx#finalization-layer).
...
- Access controls: [RBAC](./rbac.mdx) on RPC and API endpoints
...
- Learn more about selecting a
  [deployment model](../how-it-works/deployment-models.mdx)
