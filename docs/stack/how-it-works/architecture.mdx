...
As an EVM-compatible system, the Linea stack uses a dual-layer architecture with separate consensus and execution layers. This design
...
- Consensus layer: Handles block production, consensus, and block validation
- Execution layer: Executes transactions and maintains state
...
Linea is compatible with several execution layer clients:

- **[Linea Besu](../../protocol/repos#linea-besu-upstream)**: Is the recommended execution client. Includes Linea-specific plugins enabling features like
  `linea_estimateGas` and sequencer functionality
- [Geth](../../network/how-to/run-a-node/geth): Standard Ethereum client, suitable for following the chain
- [Erigon](../../network/how-to/run-a-node/erigon): Optimized for archive nodes and historical data access

All execution clients maintain the EVM state and execute transactions, but only Linea Besu provides
full Linea-specific functionality including speed optimizations.
