...
[protocol architecture](../../protocol/architecture/index.mdx).
...
The Linea stack is comprised of three broad classes of components.

### Node roles

In a fully-public network, these components run on externally reachable nodes. In other deployment types, the components may run on different nodes.
...
- RPC nodes: Expose JSON-RPC APIs to applications and users. RPC nodes may be optimized for near-head
  access or configured as archive nodes for historical queries. For details, see
  [RPC services](../../protocol/architecture/rpc-services/index.md).
...
Internal services typically communicate over operator-controlled interfaces--similar to the [Engine API](#full-nodes) used between consensus and execution clients. Traffic between these services remains within the operator’s trusted network boundary and does not traverse public networks.
...
transactions and maintains the EVM state. 
...
The [traces generator](../../protocol/architecture/sequencer/traces-generator.mdx) generates execution
traces required for proof generation.  
...
2. Block building: the[sequencer](../../protocol/architecture/sequencer/index.mdx) orders and executes transactions into blocks.
3. State tracking: the [state manager](../../protocol/architecture/state-manager.mdx) updates the state representation;
   the [tracer](../../protocol/repos.mdx#linea-tracer) generates traces.
4. Conflation: the [coordinator](../../protocol/architecture/coordinator.mdx) groups blocks into batches.
5. Proof generation: the [prover](../../protocol/architecture/prover/index.mdx) generates execution proofs for batches.
6. Blob creation: the [coordinator](../../protocol/architecture/coordinator.mdx) combines batches into blobs; 
the [prover](../../protocol/architecture/prover/index.mdx) generates compression proofs.
7. Finalization: the [coordinator](../../protocol/architecture/coordinator.mdx) submits proofs to the finalization 
...
[Web3Signer](https://docs.web3signer.consensys.io/) is a remote signing service for Ethereum and other blockchain clients that externalizes private key management from the nodes themselves. It exposes HTTP‑based signing APIs that allow consensus and execution clients to obtain signatures on blocks, attestations, and transactions without directly holding the private keys. 

In this architecture, the components participating in block production and finality integrate with Web3Signer to sign consensus messages and transactions, so that signing operations are centralized, access‑controlled, and auditable rather than embedded in each node. 

## Network topology

### Full nodes

[Full nodes](../../protocol/architecture#full-node) are composed of the execution layer client such as
...
Full nodes maintain complete network state and participate in consensus. The block is proposed by the execution layer, Linea Besu, and the consensus layer, Maru, signs and broadcasts it. Full nodes are often run with both clients in the same container. These clients communicate via the Engine API, an internal service.

### RPC nodes

[RPC nodes](../../protocol/architecture/rpc-services/index.md) provide API access to the network:

- Near-head nodes: Optimized for low-latency access to recent state
- Archive nodes: Maintain complete historical state for data availability (DA), auditing, and analysis

RPC nodes may be deployed behind load balancers and access controls (Role-Based Access control (RBAC)) for [private networks](./deployment-models/private-validium.mdx).

### Network topology patterns

Network topology varies by [deployment model](./deployment-models/index.mdx):

- [Private networks](./deployment-models/private-validium.mdx): All components run within controlled infrastructure. Data availability and access controls are
  enforced.

- [Hybrid networks](./deployment-models#hybrid): Internal services and private data availability coexist with selective public interaction.

- [Public networks](deployment-models#public): Data availability is onchain, and components interact with public infrastructure.
