...
- [**`LineaRollup` / `L1MessageService`:**](../../build/contracts#deployed-contracts)
  One deployed `LineaRollup` proxy/address combines both responsibilities.
  `L1MessageService` is inherited by `LineaRollupBase` and holds the withdrawal
  reserve (unstaked ETH that pays bridge redemptions); `LineaRollup` also
  provides the rollup and Yield Boost accounting entry points.
...
flowchart TD
    ...
    NO([Node Operator])
    LR[LineaRollup proxy\nL1MessageService withdrawal reserve]
    YM[YieldManager]
    SV[StakingVault]
    BC{{beacon chain}}
