...
For the canonical specification that this feature follows, see the
[Native Yield technical spec](https://hackmd.io/@kyzooroast/Hk7DQXH6lx).
...
- **Node operator:** Runs Ethereum validators and deposits from the `StakingVault`
  under the agreed deposit trigger.
...
- [**`LineaRollup`:**](../../build/contracts#deployed-contracts) Holds the
  withdrawal reserve (unstaked ETH that pays bridge redemptions) and provides
  the rollup and Yield Boost accounting entry points.
- **`YieldManager`:** Moves ETH between the `LineaRollup` reserve and the
...
- **`StakingVault`:** Lido V3 stVault; validator withdrawals return here before
  ETH moves back to the `LineaRollup` reserve.
...
1. **Staking** (routine): Automation moves surplus from the `LineaRollup`
...
3. **Yield reporting** (routine): After paying fees and liabilities, the
   remaining yield is reported to L2.
...
sequenceDiagram
    ...
    alt StakingVault funds available
        Note over YM: Settle obligations:<br/>stETH borrow liability, protocol fees,<br/>node operator fees
...
ETH arrives only after the beacon chain processes it. The permissionless
...
sequenceDiagram
    ...
    rect rgba(230, 240, 255, 0.6)
        ...
        Note over YM: Validate validator proof<br/>against beacon root<br/>Amount capped to remaining deficit
        YM-->>SV: triggerValidatorWithdrawals()
        SV-->>BC: request withdrawal
