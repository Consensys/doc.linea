...
| `YIELD_PROVIDER_UNSTAKER_ROLE`  | Automation Service | Rebalance ETH back into the `L1MessageService` reserve                                                                              |
| `YIELD_REPORTER_ROLE`           | Automation Service | Trigger yield reporting (may settle fees and liabilities from staking rewards)                                                      |
| `STAKING_PAUSE_CONTROLLER_ROLE` | Security Council   | Pause or resume node operator deposits                                                                                              |
| `OSSIFICATION_INITIATOR_ROLE`   | Security Council   | Begin permanent shutdown of the staking vault                                                                                       |
| `OSSIFICATION_PROCESSOR_ROLE`   | Automation Service | Finalize vault ossification                                                                                                         |
...
chain root. The proof verifies the validator container, not exit status. The
