...
Certain implementation patterns that may appear unusual in isolation are intentional design tradeoffs for operating with delayed or externally reported staking state. Reports submitted through Linea's bug bounty program should evaluate these behaviors against the documented Yield Boost architecture and the actual security impact to user funds, withdrawal availability, or yield accounting.
