...
1. **Bridging and staking**: When a user bridges ETH from Ethereum (L1) to Linea (L2) via the `LineaRollup`, a portion of these assets is programmatically deposited into Linea's Lido v3 stVault on Ethereum Mainnet. 
...
The ramp-up follows a strict phased approach described in the following steps:
...
- Duration: ~6 weeks (accounting for Ethereum’s entry and exit queues).
  1. Start with 3 initial validators (96 ETH) in non Pre-Deposit Guarantee (PDG) mode and confirm deposits and validator activation behave as expected.
  1. Add the remaining ETH up to 500 ETH to bring additional validators online; then pause staking post validator activation.

**Activation & Hardening:**
  1. Switch to PDG mode on the `StakingVault`.
  1. Exit one validator to validate the exit path and fund return.
...
Once the round-trip is validated, Linea will increase the staked percentage periodically, pending successful "staking activation" at each stage.
  1. On successful exit, switch the system to top-up mode.
  1. Unpause and resume ramp-up by topping up existing validators.
...
- No. Users retain standard, fungible 1:1 ETH on Linea. The staking process occurs in the background on the L1 within the `LineaRollup` contracts. You do not hold a wrapped or rebasing token, and your ETH balance on Linea reflects the value deposited. The system earns yield to reinforce and improve the ecosystem you are using.
