...
- As a last-resort mechanism, the user can opt to [mint stETH against the `StakingVault` collateral](../../protocol/yield-boost-architecture#5-lst-withdrawal) rather than wait for the ETH balance to be sufficient. This process is permissionless and censorship-resistant, and it is the final backstop to ensure that users can always withdraw their funds.
...
- Objective: Validate the staged activation and transition of the staking system under real conditions.
- Action: Stake 96 ETH (~$192k). 
- Duration: ~6 weeks (accounting for Ethereum’s entry and exit queues).

  1. Start with 3 initial validators (96 ETH) and confirm deposits and validator activation behave as expected.
  2. Add the remaining ETH up to 500 ETH to bring additional validators online.
  3. Once all validators are active, switch to the Protocol-Defined Guardrails (PDG)/strict flow.
  4. Exit one validator to validate the exit path and fund return.
  5. On successful exit, switch the system to top-up mode.
  6. Resume ramp-up by topping up existing validators after unpausing.
