...
Forced transactions is not enabled by default. To enable forced transactions on your smart contract, you need to:

1. Deploy an `AddressFilter` contract 
2. Deploy a `ForcedTransactionGateway` contract
...
When submitting the transaction, the gateway will validate the last finalized state matches, validate the  transaction details are within limits, the appropriate fee has been paid, and will then send it to the LineaRollup for storage.
