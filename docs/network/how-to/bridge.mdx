...
- **Native bridge**: Built by our team to provide an interface for the [canonical token bridge](../../protocol/canonical-token-bridge.mdx). 
...
In many cases, transfers initiated using the bridge aggregator are much faster than those created 
with the native bridge, but cost slightly more. In some cases, particularly when bridging to Linea 
...
(CCTP) rather than the [canonical token bridge](../../protocol/canonical-token-bridge.mdx) 
that underpins other native bridge transfers.

USDC transfers currently require [manual claiming](#manual-claiming).

The native bridge supports both standard and fast transfers for USDC. Standard transfers are the 
default option for USDC bridging, though you can select a fast transfer by clicking the "CCTP 
Standard" button displayed in the bridge:

<div class="center-container">
  <div class="img-medium">
    <img
      src="/img/get_started/how_to/bridge/bridge/Linea_native_bridge_CCTP_transfer_type.png"
      alt="Toggle the CCTP transfer type"
    />
  </div>
</div>

Refer to the table below for the expected duration of USDC transfers according to destination chain
and transfer type:
...
it manually, as it will no longer be displayed in the native bridge app. See our [message service reference page](../../protocol/message-service/reference.mdx#claim-old-messages) 
...
- L2 &rarr; L1: ~2-12 hours
- L1 &rarr; L2: ~20 minutes

See the [USDC section](#bridge-usdc) for more information on the expected duration of USDC transfers.
