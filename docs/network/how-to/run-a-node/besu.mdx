...
<<<<<<< HEAD
  ...
  config file named `config-snap-mainnet.toml` where bootnodes are specified. The file uses all
  bootnodes by default.

  The [bootnodes page](bootnodes.mdx) contains a full list of available bootnodes.
  
=======
  You can choose from a range of bootnodes for Linea Mainnet. The [bootnodes page](bootnodes.mdx) contains a full list of available bootnodes.
>>>>>>> 4f4711f6 (docs: clarify named network usage for node runners)
  :::
