...
This section is undergoing maintenance. Similarly to Ethereum, and as of Linea
[Beta v4](../../../changelog/release-notes.mdx#beta-v40), Linea uses a dual-layer architecture, with a
consensus layer client, Maru, and an execution layer client of your choice.
...
Plugins enable full functionality for the node. For example, Linea-specific API endpoints like
`linea_estimateGas` require the `linea-sequencer` plugin. Plugins are installed with the `advanced`
...
There are several `.yaml` files here corresponding to Besu profiles. Each profile enables you to run
a node with different Linea Besu plugin configurations depending on your use case.
...
[Download the latest version](https://github.com/Consensys/linea-monorepo/releases) of the Linea
