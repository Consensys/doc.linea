...
:::info important
Install and run a Nethermind client to maintain a local copy of the blockchain and serve standard
Ethereum JSON-RPC. The `linea_` namespace is implemented by Linea Besu plugins, so
