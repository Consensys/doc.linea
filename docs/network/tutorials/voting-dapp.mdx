...
This tutorial walks you through building a simple voting dapp, and you'll learn how to write, deploy and 
...
[Create Web3 Template CLI](https://npmjs.com/package/@consensys/create-web3-template) by Consensys.

This command line tool simplifies the setup process by providing all the necessary tools and
integrations out of the box, including MetaMask SDK, Linea support, Infura integration, and a selection
...
2. You'll be prompted to specify a name for your project.
  Name it `simple-voting-dapp`:
    
    ```
    ? Please specify a name for your project: simple-voting-dapp
    ```
    
3. Next, select the Next.js framework for your frontend:
...
4. Choose Hardhat as the smart contract development environment:
...
5. Select your preferred package manager. This tutorial uses pnpm:
...
The CLI will generate the project based on your selections.
...
To ensure everything is set up correctly, run the development servers.
...
### 5. Create the voting contract

Create a `Voting.sol` file and add the following code: 
...
This contract provides a basic framework for a decentralized voting system where users can cast votes
for candidates securely and transparently.
It ensures that each user can vote only once by tracking voter addresses, and it maintains a record of
all votes on the blockchain.
The contract manages the voting period by specifying start and end times, enforcing voting rules, and
providing functions to retrieve voting results and status.
...
- The `addCandidate` function allows the owner to add new candidates after hardhat deployment.
  This ensures only authorized users can modify the list of candidates.
...
In the `ignition` folder, create a file named `Voting.ts` to deploy your contract.
...
This deployment script uses Hardhat Ignition to manage the deployment of your `Voting` contract.
It sets default candidates and a default voting duration but also allows these values to be customized
through parameters if needed.
...
### 7. Configure Wagmi and MetaMask SDK
...
Since you've already set up the frontend using the Create Web3 Template CLI, you can proceed to
...
- Replace `'your_deployed_contract_address_here'` with the contract address you obtained after deployment.
...
This Next.js client-side component uses:

- React's `useState` for state management.
- Wagmi hooks (`useAccount`, `useWalletClient`, `useReadContract`, `useWriteContract`) for wallet and
  contract interactions.
- UI elements (`Button`, `Input`, `Card`, `CardContent`) from shadcn.
- A `ConnectButton` for MetaMask connection.
- `contractAddress` and `contractAbi` from constants for contract use.
...
- **Error handling**:
  - Wraps the contract interaction in a `try-catch` block to handle any errors that occur during the
    voting process.
...
The component features a styled main container with a header and a `ConnectButton` for MetaMask integration.
It conditionally renders content based on `votingStatus` and user connection: if voting is active and
the user is connected, it displays their address, remaining time, and voting options; if they've already
voted, it notifies them and lists all candidates.
If the user isn't connected, it prompts them to connect their wallet.
When voting has ended, it shows "Voting has finished."
...
You now have a decentralized voting dapp where users can vote for the candidates by using their index number.
These votes are secure, transparent and recorded on the Linea blockchain.

## Next steps

- Build on this project by adding voter registration or vote delegation, enhancing security measures, or
  improving the user interface for better accessibility.

- See the [full example dapp](https://github.com/meowyx/simple-voting-dapp) on GitHub.
