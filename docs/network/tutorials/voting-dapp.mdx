...
This tutorial will walk you through building a simple voting dapp, and you'll learn how to write, deploy and 
...
To ensure seamless interaction with the Linea blockchain, you'll use MetaMask SDK and Wagmi 
...
Create a new project using the [Create Web3 Template CLI](https://npmjs.com/package/@consensys/create-web3-template) 
by Consensys.
...
:::note Understanding the monorepo
...
functions to retrieve voting results and status.
...
It sets default candidates and a default voting duration but also allows these values to be 
...
Now you can deploy the contract by running:
...
Keep this address safe, as you'll need it when integrating with the frontend.
...
You now have a decentralized voting dapp where users can vote for the candidates by using their 
index number. These votes are secure, transparent and recorded on the Linea blockchain.

## Next steps

- Build on this project by adding voter registration or vote delegation, enhancing security 
  measures, or improving the user interface for better accessibility.

- See the [GitHub repository](https://github.com/meowyx/simple-voting-dapp) with the full example app.
