...
This tutorial will walk you thoughing building a simple marketplace dapp. 

## Architecture

The marketplace will incorporate smart contracts, a frontend, and blockchain integration.
The smart contract will include features such as listing items, buying items, and transferring ownership of an item.

The frontend serves as the user interface where buyers and sellers can interact with each other using the smart contract.
While having a frontend and backend is important, the way the dapp interacts with the Linea blockchain is also crucial.
You'll use MetaMask SDK along with Wagmi and Infura RPC providers.

## Prerequisites

- [pnpm](https://pnpm.io/) installed
- An [Infura API key](https://docs.infura.io/dashboard/get-started/create-api/)
- An account to use for contract deployments and transactions on Linea Sepolia
...
A monorepo (short for monolithic repository) is a software development strategy where code for multiple projects is
stored in a single version control repository.
...
Create the following smart contract:
...
and ensures that only valid transactions can occur.
...
You can write a script to deploy quicker by writing small commands like this on `package.json` file:

```jsx
"deploy:testnet": "npx hardhat ignition deploy ignition/modules/Marketplace.ts --network linea-testnet"
```

After the deployment, make note of the contract address.
You'll need it when integrating with the frontend. 

### 5. Set up the frontend: Next.js with shadcn/ui
...
You'll be using Wagmi and MetaMask SDK to connect your wallet and make transactions.
You just need to create a `ConnectWallet.tsx` UI component.

### 7. Add the contract to your site
...
Add the deployed contract address that you saved earlier.
Go to the `blockchain/ignition` module and find the `artifacts` folder to get the ABI.
...
`app/page.tsx` can become quite large so you can also refactor the code in the `site/src/app/components/` 
directory.
...
In this tutorial, you built a simple decentralized marketplace dapp on Linea, leveraging zkEVM technology 
...
- See the [GitHub repository](https://github.com/meowyx/web3-marketplace-linea) with the full example app.
