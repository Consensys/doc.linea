...
You'll create a simple frontend.
You can find all the code [here](https://github.com/jidohanbaiki/linea_staking).

You'll create this frontend using `rainbowkit`, which will set up a new [Next.js](https://nextjs.org/) app and is built on top of
another library called `wagmi`.
...
1. Configure the project and set up some styling.
...
Then press `enter` each time to choose the default parameters (feel free to use your preferred configuration).
...
a. You should have an `app` folder, with a `global.css` file inside.
...
b. Shadcn works by manually adding each component with the `pnpm dlx shadcn-ui@atest add <component>`, so be sure to do it for each import.
...
c. You also have a folder containing two folders `components` and `lib`.
...
e. You can delete the empty folders (these might vary depending on your system, but you should have an empty `app` folder at least).
...
1. You're ready to integrate your contracts and build the main interface.
  ...
  You can remove the content in this file and start from scratch.
    ...
    Start by defining an empty page with a connect button and some useful debug information.
...
1. Run `pnpm dev` and go to `http://localhost:3000/` in your browser.
...
2. Now that you have your frontend set up, your wallet connection working and your chain set to Linea Sepolia, you can interact
  with the contract.
...
3. For now, nothing is connected to the blockchain.
...
4. Define the contract address and ABI:
...
1. Remove the previous mock part with this new one:
...
- Optional animation:
    ...
    You can use the `isLoading` boolean to add animation while fetching values for the two inputs, "Stake" and "Unstake":
...
1. You can now handle the deposit and withdraw transactions.
...
This frontend part only demonstrated read and write operations.
Most Wagmi hooks return a lot of data such as `isLoading` or `error`, which are useful for building reactive and beautiful frontends,
so be sure to check the documentation.
This tutorial also omits error handling for simplicity.

Lastly, you can listen for events to dynamically update the frontend.
This is left as an optional exercise if you want to go further (hint: you can use
[`watchContractEvent`](https://wagmi.sh/core/api/actions/watchContractEvent) or [`watchAsset`](https://wagmi.sh/core/api/actions/watchAsset))

## Additional resources

[Solidity by Example](https://solidity-by-example.org/): useful website when creating smart contracts
