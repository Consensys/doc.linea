...
As Linea uses the London version of Ethereum, a small number of recent opcodes are
[not available](../overview/ethereum-differences.mdx).
...
You'll create a simple staking contract.
Use [Remix](https://remix.live/), an online tool that speeds up Solidity development.
For a complete development environment, you can also use [Foundry](https://www.getfoundry.sh/) or
...
1. Go to the [Remix IDE](https://remix.ethereum.org/) and create a new file in the contracts folder.
...
2. Copy and paste the following code:
    ...
    It allows you to stake any ERC-20, and contains the two basic functions `stake` and `withdraw`.
    The allowlist is set up when the contract is deployed; there are other versions where the allowlist
    can be modified, usually by the owner.
...
1. Go to the **Solidity Compiler** tab.
  Select the London version in the EVM Version list under **Advanced Configurations** (to avoid any
...
Copy the ABI using the button at the bottom.
...
3. Enter the list of tokens you want to allowlist.
  For this tutorial, three ERC-20 contracts have been deployed for your use (you can copy and paste
...
4. Select **Deploy** and confirm the transaction in MetaMask.
  You'll see success logs in the console at the bottom right.
  You can copy the contract address by going to recorded transactions (under the deploy button), but you
  can also find it by searching for your address on the
  [Sepolia Linea explorer](https://sepolia.lineascan.build).
...
Use RainbowKit to set up a new [Next.js](https://nextjs.org/) app built on top of wagmi.
These libraries simplify how to handle reading chain state, wallet connections, sending transactions,
listening for events and state changes, and more.

1. Run `pnpm create @rainbow-me/rainbowkit@latest` and choose a project name.
...
Depending on your system, you might have some differences, so use the following to ensure you have the
configuration expected by this tutorial:
...
2. shadcn works by manually adding each component with the `pnpm dlx shadcn-ui@latest add <component>`,
  so be sure to do it for each import.
...
5. You can delete the empty folders (these might vary depending on your system, but you should have an
  empty `app` folder at least).
  Also, if you have both `tailwind.config.js` and `tailwind.config.ts`, remove the `tailwind.config.js` file.
...
(saved from [step 2](#2-compile-the-contract)).
...
If you choose a token on your dapp now, the frontend automatically fetches the token decimals,
available balances, and staked balances.
...
- See the [full example dapp](https://github.com/Gwen-M/linea-first-dapp) on GitHub.
