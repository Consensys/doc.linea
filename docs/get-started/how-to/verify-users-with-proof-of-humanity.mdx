...
It provides a simple and reliable way for projects to gate access, rewards, or features to verified users.
...
2. You query the Linea PoH API to check whether the user is already verified.\
...
Recommended way to sign the message is to use wagmi's [`useSignMessage` hook](https://wagmi.sh/react/api/hooks/useSignMessage):
  ```tsx
...
4. You generate a link including this signed payload and redirect or open it in an iframe.
...
7. You then query the Linea PoH API to confirm that the attestation has been issued onchain.\
Endpoint: `https://poh-api.linea.build/poh/v2/{address}`\
Documentation [here](https://poh-api.linea.build/#/PoH/PohController_getOnePOHv2).
...
APIs presented below are free to use, without any key or authentication.
...
```typescript
const sig = (await fetch(`https://poh-signer-api.linea.build/poh/v2/${address}`)).text();
