...
- You must run **both** an execution layer (EL) client and Maru, a CL client. A standalone EL client
  will not work.
...
As a result, there is no zero-downtime upgrade path; you must plan a maintenance window.
...
- [Start Maru](#step-2-run-docker-compose) before the fork timestamp
...
- Start both [Maru and Geth simultaneously](#start-maru-and-el-client-simultaneously)
...
You must run this to start Maru _before_ the fork timestamps detailed in the [release notes](../../../release-notes.mdx#beta-v4).
