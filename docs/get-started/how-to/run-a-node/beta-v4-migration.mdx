...
description: Detailed walkthroughs and how-tos for upgrading your Linea node to the latest hard fork or other upgrade
...
Date: Monday, December 2nd at 10:00 UTC
Action: Sepolia node runners must upgrade by Monday morning
**Linea Mainnet**
Date: Wednesday, December 4th (pending successful Sepolia upgrade)
Condition: Only if Sepolia upgrade completes without issues
Action: Mainnet node runners must upgrade by Wednesday morning
Confirmation: We will announce the mainnet go/no-go on Monday evening
...
**Note:** Besu shows Osaka in `.result.next.forkId`, while Geth shows it in `.result.last.forkId`. Both should show `0x24ef0ae2` for Osaka.
