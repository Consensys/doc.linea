...
Besides onboarding users, devs can also set up the built-in Arcana wallet to use the preconfigured Linea chain as the default active chain in the wallet. This lets authenticated users to access the Arcana wallet and review, sign Linea transactions without having to manually set up and configure a wallet before using the Linea chain.  
