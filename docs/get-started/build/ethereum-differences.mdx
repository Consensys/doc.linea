...
Linea. Absence from this page indicates that the functionality is available on Linea and identical 
to behavior on Ethereum Mainnet; if you're experiencing otherwise, please get in touch via Discord.
...
{/* TODO: Include description of BEACONROOT's functionality on Mainnet / context with Linea */}
        ...
        <td>Available on Linea with Beta v4; the value returned is to be determined.</td>
    </tr>
        <tr>
        <td>`BLOBBASEFEE`</td>
        <td>Returns the value of the blob base-fee of the current block.</td>
        <td>Will always return the minimum value.</td>
        <td>Available on Linea with Beta v4.</td>
    </tr>
    <tr>
        <td>`BLOBHASH`</td>
    ...
    <tr>
        ...
        <td>Identical to Ethereum.</td>
        <td>Supported with Linea Beta v4.</td>
    </tr>
    <tr>
        <td>`PREVRANDAO`</td>
    ...
    <tr>
        ...
        Solidity compiler version 0.8.20, which came after the London release. Available on Linea 
        with Beta v4.</td>
    </tr>
    <tr>
        <td>`TLOAD`</td>
...
Linea's sequencer limits the size of call data, so that transactions can fit into the corresponding blob.
The current limit is 60000 bytes.

## Precompiles

### `BLAKE2f`

`BLAKE2f` is not supported on Linea.
...
The point evaluation precompile was introduced in [EIP-4844](https://eips.ethereum.org/EIPS/eip-4844#point-evaluation-precompile). 
It is available on Linea with Beta v4.
...
`RIPEMD-160` is not supported on Linea.
...
The calldata field is limited to 60,000 bytes. 