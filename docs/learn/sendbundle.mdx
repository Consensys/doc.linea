---
title: Recover tokens with eth_sendBundle
description: A guide to learn how to use eth_sendBundle on Linea.
image: /img/socialCards/recover-tokens-with-ethsendbundle.jpg
---
...
For an overview of `eth_sendBundle` on Linea, see the [reference page](../api/reference/eth-sendbundle.mdx).
...
```jsx title="components/ConnectWallet.tsx"
'use client';
...
```jsx title="components/RecoveryForm.tsx"
'use client';
...
```jsx title="constants/erc20ABI.ts"
...
2. **Get test ETH for the safe wallet:** Use a faucet to acquire Linea Sepolia ETH for Account A (safe wallet). The [official Linea docs](../get-started/how-to/get-testnet-eth.mdx) suggest using the MetaMask faucet (0.5 ETH/day) or other community faucets.
3. **Get test tokens into the hacked wallet:** You need some ERC-20 tokens in Account A (hacked wallet) while it has 0 ETH. You can use the Linea testnet token faucet. For example, you can reuse the tokens we deployed for you in our [first tutorial](./first-dapp.mdx). 
...
We hope you found this guide useful. By practicing on testnet and [following security best practices](https://support.metamask.io/stay-safe/protect-yourself/fighting-back-against-sweeper-bots), we hope you know have a better knowledge about token recovery solutions using private transaction bundles on Linea. Good luck, and stay safe out there in Web3!
