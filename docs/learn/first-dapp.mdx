---
title: How to build on Linea: a step-by-step tutorial to deploy your first dApp
description: A step-by-step guide to building your first dApp on Linea.
---

## Gm developers 👋

Today we're going to learn together how to create a dapp that will allow a web3 degen to stake certain tokens in your contract! 🔥

We'll be using the Linea blockchain ⛓️ for this tutorial. It's a zk-rollup EVM equivalent, fully EVM-compatible and 100% proven, with advantages such as high speed and low costs.

You can get some Linea Sepolia (testnet) ETH [here](https://www.infura.io/faucet/linea). 

> ℹ️ As Linea uses the **London** version of Ethereum, a small number of recent opcodes are not available.
> 
...
This is a simplified version of a staking contract with whitelist, so let's take a moment to explain it:

- This contract allows you to virtually stake any ERC20, and contains the two basic functions "stake" and "withdraw".
- The whitelist is set up when the contract is deployed; there are other versions where the whitelist can be modified, usually by the owner.

![First two steps (1 & 2)](/img/learn/first-dapp/tutoremix_1.png)
...
2. Select your injected provider (for us, Metamask) as environment. You'll get a connection request that you'll have to accept.
    - What should I do if my injected provider doesn't appear? 🆘
        ...
        It may happen that your injected provider does not appear by default. If this happens to you, follow these steps:
        ...
        ![Troubleshoot: injected provider not in Environment list](/img/learn/first-dapp/tutoremix_troubleshoot1.png)
        
        Troubleshoot: injected provider not in Environment list
        
3. Check that the contract is the one you want to deploy.
4. Enter the list of tokens you want to whitelist. For this tutorial, I've deployed 3 ERC20 contracts that you can use (you can copy and paste directly into the field): 
...
Those ERC20 contracts are verified so you can interact with them directly on LineaScan to mint some tokens. Put your address and the number of tokens you want (don't forget that if you want 1 token you have to put 1e18, e.g., 100000000000000000)
...
![Steps 6 to 10](/img/learn/first-dapp/tutoremix_3.png)
...
e. Finally we can delete the empty folders (might vary depending on your system, but you should have an empty `app` folder at least). Also, if you have both `tailwind.config.js` and `tailwind.config.ts`  you should remove the useless `tailwind.config.js` file.

The configuration is finally done, you should have a project structure like:
...
1. We are now ready to integrate our contracts and build the main interface! We'll do that in the `src/pages/index.tsx` file that will be our main page. You can remove the content in this file and start from scratch.
...
1. You can now run `pnpm dev` and go to [`http://localhost:3000/`](http://localhost:3000/) in your browser. Try to connect your wallet, rainbowKit should automatically ask you to switch to Linea Sepolia. Verify that chainId is `59141` in the debug information corner.
2. Now that we have our frontend set up, our wallet connection working and our chain set to Linea Sepolia, we can move to the next part: interacting with the contract 🥳
    ...
    Copy and paste this code on top of `index.tsx`, it's a simple staking card design:
    ...
    We'll not explain much this code as it's mostly React state management. We define the list of tokens as a constant to simplify. The `StakingCard` component handles all the token selection and inputs as well. 
...
3. For now, nothing is connected to the blockchain. You can note that the `mockFetchAmountStakableForToken` and `mockFetchAmountStakedForToken`  functions just return a constant. We will use the `useReadContracts` hook for this purpose. We are also using `useWatchBlockNumber` to re-read the state on each new block.
...
1. Now you can remove the previous mocked part with this new one:
...
- Optional animation:
    ...
    We can use the `isLoading` boolean to add some animation when we are fetching on the two Inputs of Stake and Unstake:
...
And voilà! 🎉 You should be able to easily stake and unstake from the list of whitelisted tokens.

This frontend part only demonstrated read and write operations. Most hooks of wagmi return lots of data such as `isLoading` or `error`, which is really useful for building reactive and beautiful frontends so be sure to check the documentation. We also omit error handling here for simplicity.

Lastly, we can listen for events to dynamically update the frontend. So I'll let this part as an exercise to go further (hint: you can use [watchContractEvent](https://wagmi.sh/core/api/actions/watchContractEvent) or [watchAsset](https://wagmi.sh/core/api/actions/watchAsset))
...
[Linea Docs](https://docs.linea.build): where you can find more informations about Linea

[Solidity by Example](https://solidity-by-example.org/): really useful website when you want to create a smart-contract
