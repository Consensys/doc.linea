...
- This contract allows you to virtually stake any ERC-20, and contains the two basic functions "stake" and "withdraw".
...
3. For now, nothing is connected to the blockchain. Note that the `mockFetchAmountStakableForToken` and `mockFetchAmountStakedForToken`  functions just return a constant. We will use the `useReadContracts` hook for this purpose. We are also using `useWatchBlockNumber` to re-read the state on each new block.
