...
- Upgraded Linea to Fusaka (Ethereum's latest EVM upgrade), detailed in the [upgrade guide](../public/how-to/run-a-node/beta-v4-migration.mdx).
...
- Updated the burn mechanism to include LINEA tokens. Details and percentage breakdown in the [Burn Mechanism guide](../technology/burn.mdx).
...
precompiles, such as `PUSH0` and `MCOPY`, considerably shortening the list of Linea's [differences with Ethereum](../public/build/ethereum-differences.mdx).
...
Beta v4.0 introduces a new consensus client, [Maru](../public/how-to/run-a-node/maru.mdx), 
...
- Introduces [state recovery](../technology/state-recovery.mdx), enabling anyone to recover 
...
See our [guide](../public/how-to/finalized-block.mdx) for more.
...
securely querying data on Linea. See our [page on CCIP Read](../public/tools/cross-chain/ccip-read.mdx) 
