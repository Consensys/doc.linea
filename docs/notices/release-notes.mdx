...
displayed_sidebar: noticesSidebar
...
- Upgraded Linea to Fusaka (Ethereum's latest EVM upgrade), detailed in the [upgrade guide](../network/how-to/run-a-node/beta-v4-migration.mdx).
...
- Updated the burn mechanism to include LINEA tokens. Details and percentage breakdown in the [Burn Mechanism guide](../protocol/burn.mdx).
...
precompiles, such as `PUSH0` and `MCOPY`, considerably shortening the list of Linea's [differences with Ethereum](../network/build/ethereum-differences.mdx).
...
- Reworks [prover limits](../protocol/prover/prover-limits.mdx) to reduce infrastructure costs and 
...
- Introduces [state recovery](../protocol/state-recovery.mdx), enabling anyone to recover 
...
securely querying data on Linea. See our [page on CCIP Read](../network/tools/cross-chain/ccip-read.mdx) 
...
Major changes are applied to the Canonical Token Bridge as described in our [documentation](../protocol/canonical-token-bridge.mdx).
