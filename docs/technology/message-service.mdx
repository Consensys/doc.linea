...
The canonical message service consists of three main elements: two smart contracts, and the postman service in between. As you may have guessed already, the smart contracts are [on Linea and Ethereum](./message-service.mdx#message-service-contracts), and are almost exactly the same. They allow for ETH to be minted on the target network, for example, though they are not limited to that.
...
Two methods are available for sending messages across chains:

- Interacting directly with the message service proxy contract on the origin chain.
- Using the [Linea SDK](../api/linea-sdk.mdx).

#### Use the message service contract

Call `sendMessage(...)` on the origin chain using the proxy contract at one of the addresses 
[above](#message-service-contracts), depending on the chain.
...
If messages don't get delivered by the postman, you can trigger their delivery (trigger a manual
claim) with one of two options:

1. Run the postman with event filtering configuration that instructs it to listen only for 
messages that meet certain criteria. For example, a dapp could filter to ensure the postman only 
listens for events that originate from its users.
2. Use the parameters detailed in the [interface below](#interface-imessageservicesol) to call
  one of:
    - L2: `claimMessage` 
...
- Receiving the delivery in the dapp smart contract
  ...
  - The dapp smart contract can inherit from `MessageServiceBase.sol` to:
    - Verify that the call comes from the message service with `onlyMessagingService`
