...
`.env` file (available as `env.sample` when you clone the monorepo): 
    ...
    parameters names in order to use syntax like `calldata.params.messageNumber`.

2. **`claimMessage`**: If you know the specific details of the message, call one of these methods 
using the parameters detailed in the [interface below](#interface-imessageservicesol):
    - L2: `claimMessage` 
...
Refer to the [SDK README](https://github.com/Consensys/zkevm-monorepo/tree/main/sdk#readme) for 
