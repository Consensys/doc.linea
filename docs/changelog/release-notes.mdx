...
<div id="beta-v54" className="legacy-anchor" /><div id="beta-v53" className="legacy-anchor" />

<details open className="upcoming-releases">
<summary>Upcoming releases &mdash; 2 planned</summary>

Timings are targets and can move.

- **Beta v5.3** &middot; Prover performance optimizations &mdash; Linea Mainnet Q4 2026
- **Beta v5.4** &middot; Finality under 30min &mdash; Linea Mainnet Q4 2026
...
- Switches prover to small fields, enabling faster proof generation. **This change deprecates the
...
Upgrades Linea to Fusaka (Ethereum's latest EVM upgrade), detailed in the [upgrade guide](../network/how-to/run-a-node/beta-v4-migration.mdx).
...
Upgrades to limitless prover, enabling higher throughput and lower gas fees.

Increases throughput to 100 Mgas/s.
...
[burn mechanism guide](../network/overview/tokenomics.mdx#burning-mechanism).
...
- Boost prover performance by reducing proving time by 33%, supporting scaled trading, consumer, and
...
<ChangelogDate>
<div className="changelog-date"><span className="changelog-date__arrow">→</span><span>Linea Mainnet:</span></div>

- Paris: 22 October, 2025, [block 24787631](https://lineascan.build/block/countdown/24787631)
- Shanghai: 10:00 UTC, 23 October, 2025
- Cancun: 10:00 UTC, 28 October, 2025
- Prague: 10:10 UTC, 28 October, 2025

<div className="changelog-date"><span className="changelog-date__arrow">→</span><span>Linea Sepolia:</span></div>
...
<ChangelogEntry tag="upgrade" title="Pectra hard fork">

This release will see Linea implement approximately four years' of Ethereum protocol upgrades in a
matter of weeks: from the London hard fork beforehand this release, all the way through to Prague.
...
[differences with Ethereum](../network/overview/ethereum-differences.mdx).
...
Beta v4.0 introduces a new consensus client, [Maru](../network/how-to/run-a-node/maru.mdx),
replacing the existing Proof-of-Authority sequencer mechanism, Clique. Maru introduces a QBFT
consensus mechanism, and will support Linea's future transition to decentralized sequencing.
Initially, only a single Maru node operated by Linea will produce blocks.
...
- Made Linea's prover 2x more efficient, reducing the cost of running the Linea infrastructure. In
  future, this additional efficiency will increase ETH and LINEA burn
...
Note that the higher throughput will not be enabled until the sequencer limits are removed in [Beta v3.1](#beta-v31).
...
100% of EVM operations are covered by our zero-knowledge proofs, guaranteeing
the integrity of all onchain data when sequencing is decentralized. Linea is the first zkEVM
to be completely proven.
- Reworks [prover limits](../protocol/architecture/prover/prover-limits.mdx) to reduce infrastructure costs and
...
Introduces [state recovery](../network/overview/recoverable-state.mdx), enabling anyone to recover
...
- Proof of all the activated modules, excluding the hub consistency arguments.
...
100% of the zkEVM specification, an objective completed in Beta v2.
...
Introduces the [Linea Token API](../api/token-api/) at alpha stage.
...
<ChangelogEntry tag="upgrade" title="Linea native bridge UI v2">
...
See our [guide](../network/overview/transaction-finality.mdx#how-to-check-finality) for more.
...
Reactivates [`linea_estimateGas`](../api/reference/linea-estimategas.mdx), disabling compatibility
mode.
...
securely querying data on Linea. See our [page on CCIP Read](../network/build/tools/cross-chain/ccip-read.mdx)
...
See our [reference page](../api/reference/linea-estimategas.mdx) for more information and usage.
...
always be able to claim bridged funds. Bridge transfers to alternative recipients (that is, an address
...
- Adds the [`eth_sendRawTransaction`](../api/reference/eth-sendrawtransaction.mdx) method,
...
<ChangelogEntry tag="upgrade" title="EIP-4844">
...
**March 26:** Begin using blobs to post compressed L2 data to L1, with the aim of reducing data availability
...
<details>
  ...
  The Linea prover can operate with a compressed input, meaning it
  ...
  Linea averaged ~150 transactions per batch; afterward, it averages ~1,500.
...
their claiming outside of the Bridge UI, then we recommend looking at the updated [SDK](../api/linea-sdk.mdx).
...
This entry covers Linea's June 2023 Goerli testnet release. The contracts below are
testnet contracts and have since been superseded. Audit reports for Linea Mainnet
contracts are listed on the [Security audits](../protocol/reference/security-audits.mdx) page.
...
- **L1 (Goerli)**:
  ...
  - Bridging partners, to send messages, will need to call the new contract address with a different ABI.
    ...
    ABI: [zkEVM2.abi](/files/testnet/2023-06-13-release/zkEVM2.abi)
...
- **L2 (Linea)**:
  ...
  - Bridging partners, to send messages, will need to call the new contract address with a different ABI.
    ...
    ABI: [L2MessageService.abi](/files/testnet/2023-06-13-release/L2MessageService.abi)
...
- **L1 (Goerli)**
  ...
  ABI: [TokenBridgeL1.abi](/files/testnet/2023-06-13-release/TokenBridgeL1.abi)
...
- **L2 (Linea)**
  ...
  ABI: [TokenBridgeL2.abi](/files/testnet/2023-06-13-release/TokenBridgeL2.abi)
