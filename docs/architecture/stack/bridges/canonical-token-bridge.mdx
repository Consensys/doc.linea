...
The canonical token bridge is the pair of “lock & mint” contracts that allow bridging of any ERC-20 token. The bridge relies on the [message service](../canonical-msg-service/message-service.mdx) for cross-chain interactions.
...
To find out which bridges are currently operating on Linea, head to the [ecosystem portal](https://linea.build/apps) and click on the **Bridge** button to show all that are available.
