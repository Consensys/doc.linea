...
1. Dapp uses the [Postman SDK](https://www.npmjs.com/package/@consensys/linea-sdk?activeTab=readme) to simplify the execution of messages on the destination layer by:
   - Triggering the delivery
     - If messages don't get delivered by the postman, the message can be manually claimed by calling, with the parameters detailed in the [interface below](#interface-imessageservicesol), one of:
     - L2: `claimMessage`
