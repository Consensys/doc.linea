...
The Canonical Message Service consists of three main elements: two smart contracts, and the Postbots service in between. As you may have guessed already, the smart contracts are [on Linea and Ethereum](https://docs.linea.build/architecture/bridge-architecture/message-service#contracts), and are almost exactly the same. They allow for ETH to be minted on the target network, for example, though they are not limited to that.
