...
As a result, Linea will achieve type 2 EVM-equivalence. Due to the complexity of this process, the 
upgrade will occur in stages, progressing through hard forks according to the release schedule above.

For Linea users, the upgrade will enable new use cases and experiences through EIP-7702 (coming in [Beta v4.1](#beta-v41)), unlocking access to apps that rely on this capability.
...
EIP-7702 will only be activated in [Beta v4.1](#beta-v41)

:::

### Maru consensus client

Beta v4 introduces a new consensus client, Maru, replacing the existing Proof-of-Authority sequencer 
mechanism, Clique. Maru introduces a QBFT consensus mechanism, and will support Linea's future 
transition to decentralized sequencing. Initially only a single Maru node operated by Linea will 
produce blocks. See our [guide for node runners on upgrading](./get-started/how-to/run-a-node/beta-v4-migration.mdx) to this system.
