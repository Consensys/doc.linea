---
title: Canonical Token Bridge
sidebar_position: 1
---
import CodeBlock from "@theme/CodeBlock";
import TokenBridge from "!!raw-loader!../contracts/TokenBridge.sol";
import TokenBridgeL1 from "!!raw-loader!../abis/canonical-token-bridge/TokenBridgeL1.abi";
import TokenBridgeL2 from "!!raw-loader!../abis/canonical-token-bridge/TokenBridgeL2.abi";

# Canonical Token Bridge

The Canonical Token Bridge is the “lock & mint” contracts that allow bridging to any ERC20, which relies on the [Message Service](./message-service.mdx) for cross-chain interactions and does not come with any UI.

## Contracts

- **L1 (Goerli)**

  <details>
    <summary>TokenBridge.abi</summary>
    <CodeBlock language="json">{TokenBridgeL1}</CodeBlock>
  </details>

  - Contracts:
    - [Transparent Proxy](https://goerli.etherscan.io/address/0xaA012D038E6440535Ec66eDf2DA592F4F8398133)
    - [Implementation](https://goerli.etherscan.io/address/0x03c73b65d5c2e135df0e42331f41fbe1590f6486#code)

- **L2 (Linea)**

  <details>
    <summary>TokenBridge.abi</summary>
    <CodeBlock language="json">{TokenBridgeL2}</CodeBlock>
  </details>

  - Contracts:
    - [Transparent Proxy](https://explorer.goerli.linea.build/address/0xB191E3d98074f92584E5205B99c3F17fB2068927)
    - [Implementation](https://explorer.goerli.linea.build/address/0xe24199fba294c90e4DACF3cAA1C5F8eeB726E85a)

## How to use

### Workflow

1. Triggering the transfer

- **Without Permit**
  - User should first allow the bridge to transfer tokens on his behalf
  - This is done by calling `allowance()` on the TokenBridge.
  - Then, the user should call the `bridgeToken()` method.
- **With Permit**
  - User can call `bridgeTokenWithPermit()` to pass permit data in a single transaction

1. Triggering the delivery

- See [Message Service step](./message-service.mdx#workflow)

## Interface TokenBridge.sol

<details>
  <summary>TokenBridge.sol</summary>
  <CodeBlock language="solidity">{TokenBridge}</CodeBlock>
</details>
