---
title: How do I bridge ERC20 tokens?
sidebar_position: 2
---


:::note

We recommend that only tech operators that are providing liquidity use our bridge for ERC20 tokens. For everyone else, please take a look at our [ecosystem portal](https://linea.build/apps) and filter for bridges.
**ERC20 tokens can only be bridged via manual claiming, which requires you to cover the ETH fee on the destination layer. Make sure your wallet is properly funded before beginning this process! Check out the [tutorial](/use-mainnet/bridges-of-linea/how-to-bridge-eth) on how to bridge ETH to learn how to use our bridge and appropriately fund your wallet.**


If you're bridging a token to Linea that hasn't been deployed before and you see a difference in the transferred token amount, don't worry – your tokens aren't lost. This could be a "token bridge decimal mismatch," which affects how the value is displayed. The chance of encountering this is quite low, but still good to know about. To learn what "token bridge decimal mismatch" is click [here](/use-mainnet/bridges-of-linea#what-is-token-bridge-decimal-mismatch).


:::

1. Go to our token-bridge [here](https://bridge.linea.build/).

2. Connect your wallet in the top-right corner of the page.

3. On the [token bridge](https://bridge.linea.build/), go to the token selection menu by clicking on the current token symbol (ETH in this case).

<img
  src={
    require("@site/static/img/docs/use-mainnet/bridges-of-linea/token_selection_menu.png")
      .default
  }></img>

4. Select one of the available tokens in the list. If the token is not in the list, paste the address of the token you want to add into the search bar.

<img
  src={
    require("@site/static/img/docs/use-mainnet/bridges-of-linea/token_list.png")
      .default    
  }></img>


5. Click on the token to select it.

<img
  src={
    require("@site/static/img/docs/use-mainnet/bridges-of-linea/dogecoin.png")
      .default
  }></img>


6. You now can bridge this token, and it will be saved in your token list for future bridging.

<img
  src={
    require("@site/static/img/docs/use-mainnet/bridges-of-linea/doge_bridging.png")
      .default
  }></img>
