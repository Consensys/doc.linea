...
The message number is preset (`nextMessageNumber`) and only incremented at the end if successful for the next caller.
This function should be called with a `msg.value` equal to `_value` plus `_fee`. The fee will be paid on the destination chain.
