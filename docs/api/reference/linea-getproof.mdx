...
- `storageProofs`: An array of storage-entries as requested. Each entry is an object with these properties:
...
:::note
The response below is illustrative. Proof bytes (`leafIndex`, `value`, and the contents of
`proofRelatedNodes`) are tied to the merkle tree state at the requested block and will differ for
