...
Array of trace objects across all transactions in the block. Each trace includes `transactionHash`
and `transactionPosition` to identify which transaction it belongs to. See
[`trace_transaction`](trace-transaction) for the trace object shape.
