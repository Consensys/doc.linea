...
:::info
The following pages document every JSON-RPC method available on the Linea public RPC endpoint
(`https://rpc.linea.build`). Methods with Linea-specific behavior are documented in detail.
Standard Ethereum methods include `curl` examples you can run directly against the public endpoint,
no API key required.
...
peer counts, client versions) will differ when you run the same calls. This is expected behavior,
not an error.
:::
